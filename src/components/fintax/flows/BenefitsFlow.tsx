...
export function BenefitsFlow() {
  ...
  const persistCurrentSnapshot = React.useCallback(async () => {
    ...
    try {
      ...
      setDraftCaseId((current) => current ?? updated.id);
  ...
  return (
    <div className="space-y-6">
      ...
      {serverState === "fallback" && !draftCaseId ? (
        <div className="rounded-2xl border border-border/45 bg-surface2/60 px-4 py-3 text-sm text-secondary">
          Local draft is active until the first server-backed case is created.
        </div>
      ) : null}
