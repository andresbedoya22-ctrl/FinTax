...
import { Activity, CheckCircle2, Circle, FileUp, ShieldCheck } from "lucide-react";
    ...
    return (
      <Card>
        <CardBody className="text-sm text-secondary">
          {caseQuery.isError ? t("loadError", { code: caseErrorCode ?? "unknown" }) : t("notFound")}
        </CardBody>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <Badge variant="copper">{t(`caseTypeLabels.${caseItem.case_type}`)}</Badge>
              <span
                className={cn(
              ...
              >
                {t(`statusLabels.${caseItem.status}`)}
              </span>
            </div>
        ...
        <TabsContent value="documents">
          <Card>
            ...
            <CardBody className="space-y-4">
              ...
              {docs.length === 0 ? (
                <div className="rounded-xl border border-border/35 bg-surface2/15 p-4 text-sm text-secondary">{t("documents.empty")}</div>
              ) : null}
      ...
      <Card>
        ...
        <CardBody>
          {checklist.length === 0 ? (
            <div className="rounded-xl border border-border/35 bg-surface2/15 p-4 text-sm text-secondary">{t("checklist.empty")}</div>
          ) : (
            <ul className="space-y-2">
              {checklist.map((item, idx) => (
                <li key={item.id} className="flex items-center gap-3 rounded-xl border border-border/35 bg-surface2/20 px-4 py-3 text-sm">
                  {item.is_completed ? <CheckCircle2 className="size-4 text-green" /> : <Circle className="size-4 text-muted" />}
