    ...
    <UiNavbar
      ...
      brand={
        <Link href="/" className="focus-ring inline-flex items-center rounded-lg text-text">
          <span className="mr-2 grid h-7 w-7 place-items-center rounded-xl border border-green/35 bg-green/10 text-xs font-black text-green">
            F
          </span>
      ...
      nav={
        <>
          {links.map((link) => (
            <Link key={link.key} href={link.href} className="focus-ring rounded-lg px-3 py-2 text-sm text-secondary hover:text-text">
              {t(`links.${link.key}`)}
            </Link>
          ))}
        </>
      ...
      actions={
        <>
          <LanguageSwitcher />
          <Link href="/auth" className="focus-ring rounded-lg px-2 py-2 text-sm text-secondary hover:text-text">
            {t("signIn")}
          </Link>
