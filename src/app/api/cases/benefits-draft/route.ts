...
import { createBenefitsDraftPayload, isBenefitsCaseType, type BenefitsDraftPayload, type BenefitsDraftRequest } from "@/lib/benefits/contracts";
...
async function parseBenefitsDraftRequest(request: Request): Promise<BenefitsDraftRequest | null> {
  const body = (await request.json().catch(() => null)) as
    | {
        caseId?: unknown;
        preferredCaseType?: unknown;
        payload?: Partial<BenefitsDraftPayload>;
      }
    | null;
  ...
  return {
    ...
    preferredCaseType:
      typeof body.preferredCaseType === "string" && isBenefitsCaseType(body.preferredCaseType)
        ? body.preferredCaseType
        : undefined,
    payload: createBenefitsDraftPayload(body.payload),
  };
}
...
export async function POST(request: Request) {
  ...
  const parsed = await parseBenefitsDraftRequest(request);
  if (!parsed) return apiError("invalid_payload", "invalid_benefits_draft");
  ...
  if (existing.data) {
    const { data: updated, error } = await authed.supabase
      .from("cases")
      .update(buildBenefitsUpdateInput(existing.data, parsed))
  ...
  const { data, error } = await authed.supabase
    .from("cases")
    .insert(buildBenefitsInsertInput(authed.user.id, parsed))
    .select(BENEFITS_CASE_SELECT)
    .single();
...
export async function PATCH(request: Request) {
  ...
  const parsed = await parseBenefitsDraftRequest(request);
  if (!parsed?.caseId) return apiError("invalid_payload", "invalid_benefits_draft");
  ...
  const { data, error } = await authed.supabase
    .from("cases")
    .update(buildBenefitsUpdateInput(existingCase, parsed))
    .eq("id", parsed.caseId)
    .eq("user_id", authed.user.id)
    .select(BENEFITS_CASE_SELECT)
    .maybeSingle();
